...
export default function RawMaterialListPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => await api.delete(`/item-setup/raw-material/${id}`),
  ...
  const filtered = rawMaterials?.filter(rm => rm.name.toLowerCase().includes(searchTerm.toLowerCase())) || [];
      ...
      <Card className="dark:bg-[#111827] dark:border-slate-800">
        <CardContent className="p-0">
          <div className="p-4 flex justify-between items-center border-b dark:border-slate-700">
            <Link to="/setup/raw-material/add" className="flex items-center px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md text-sm font-medium">
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {paginated.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="text-center py-8">No raw materials found</TableCell></TableRow>
                ) : (
                  paginated.map((item, i) => (
                    <TableRow key={item.id} className="dark:border-slate-700">
                      ...
                      <TableCell className="text-right space-x-2">
                        <Link to={`/setup/raw-material/edit/${item.id}`} className="inline-block text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 p-1 rounded"><Edit className="w-4 h-4" /></Link>
