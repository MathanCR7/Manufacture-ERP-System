...
export default function AddProductPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (isEditMode) return (await api.put(`/item-setup/product/${id}`, data)).data;
      return (await api.post('/item-setup/product', data)).data;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(data => mutation.mutate(data))}>
        <Card className="dark:bg-[#111827] dark:border-slate-800">
          <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <input {...register('code', { required: true })} placeholder="e.g. PRD-001" className="w-full px-3 py-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 dark:text-white" />
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Selling Price (INR)</label>
        ...
        <div className="mt-6 flex space-x-4">
          ...
          <button type="button" onClick={() => navigate('/setup/product')} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-md font-medium">
            Back
          </button>
        </div>
      </form>
