...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Includes debounced search input, type filter tabs, category & UOM badges.
   Exact replica of /purchase-orders/create selector with 200ms debounce.
   ───────────────────────────────────────────────────────────────────────────── */
const RawMaterialSelect = React.forwardRef(function RawMaterialSelect(
  ...
  }, [rawMaterials, typeFilter, debouncedSearch]);
        ...
        <div className="absolute z-50 mt-1.5 left-0 right-0 w-full max-w-[calc(100vw-1.5rem)] sm:max-w-none bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden animate-in fade-in-50 zoom-in-95 duration-150 ring-1 ring-black/5 dark:ring-white/5">
          ...
          <div className="p-2 border-b border-slate-100 dark:border-slate-800 relative bg-slate-50/70 dark:bg-slate-950/70">
            ...
            <input
              ...
              className="w-full pl-8 pr-14 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-slate-900 dark:text-white"
            />
          ...
          <ul className="max-h-56 sm:max-h-64 overflow-y-auto p-1.5 space-y-1">
            {filtered.length === 0 ? (
              <li className="px-3 py-6 text-xs text-slate-400 flex flex-col items-center justify-center">
                ...
                <span className="font-medium">No items matched "{debouncedSearch || search}"</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 flex-wrap sm:flex-nowrap pl-4 sm:pl-0">
                      ...
                      {/* Available Stock or Rate */}
                      {rm.itemType === 'RAW_MATERIAL' && rm.currentStock !== undefined && (
                        <span className="text-[10px] font-mono font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">
                          Avail: {Number(rm.currentStock).toFixed(1)} {uomLabel}
                        </span>
                      )}
...
function ProductForm({ editId, onBack }) {
  ...
  // Searchable Item Selection State (Raw Materials & Non-Inventory)
  const [selectedItem, setSelectedItem] = useState(null);
  const [itemError, setItemError] = useState(false);
  const rmSelectRef = useRef(null);

  // Combined selectable catalog items with Category & UOM metadata (matching /purchase-orders/create)
  const allSelectableItems = useMemo(() => {
    const rms = (masters.rawMaterials || []).map(rm => ({
      ...rm,
      itemType: 'RAW_MATERIAL',
      itemTypeLabel: 'Raw Material',
      categoryName: rm.category?.name || (typeof rm.category === 'string' ? rm.category : '') || rm.categoryName || 'General',
      displayUom: getUomLabel(rm.consumptionUnit || rm.unitId),
      isLowStock: Number(rm.alertLevel) > 0 && Number(rm.currentStock || 0) <= Number(rm.alertLevel),
      currentStock: Number(rm.currentStock || 0),
      ratePerUnit: Number(rm.ratePerUnit || 0),
    }));

    const nonInvs = (masters.nonInventoryItems || []).map(ni => ({
      ...ni,
      ...
      isLowStock: false,
      currentStock: 0,
      ratePerUnit: Number(ni.ratePerUnit || ni.cost || 0),
    }));

    return [...rms, ...nonInvs];
  }, [masters.rawMaterials, masters.nonInventoryItems, masters.units]);

  // Set of low stock item IDs
  const lowStockIds = useMemo(() => {
    return new Set(
      allSelectableItems
        .filter(it => it.itemType === 'RAW_MATERIAL' && it.isLowStock)
        .map(it => it.id)
    );
  }, [allSelectableItems]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [mastersRes, rmRes, nonInvRes] = await Promise.all([
          api.get('/products/masters'),
        ...
        const rawMaterials = (rmRes.data && Array.isArray(rmRes.data) && rmRes.data.length > 0) ? rmRes.data : (mastersData.rawMaterials || []);
        const nonInventoryItems = (nonInvRes.data && Array.isArray(nonInvRes.data) && nonInvRes.data.length > 0) ? nonInvRes.data : (mastersData.nonInventoryItems || []);
  ...
  // BoM Handlers (Raw Materials & Non-Inventory Items)
  const handleAddItem = () => {
    if (!selectedItem) {
      setItemError(true);
      return;
    ...
    if (selectedItem.itemType === 'RAW_MATERIAL') {
      if (bom.some(item => item.rmId === selectedItem.id)) {
        Swal.fire({
          title: `<span class="font-extrabold text-sm text-slate-800 dark:text-slate-100">Already Assigned</span>`,
          text: `Raw material "${selectedItem.name}" is already assigned to the BoM.`,
          icon: 'warning',
          toast: true,
          position: 'top-end',
      ...
      const uomLabel = selectedItem.displayUom || getUomLabel(selectedItem.consumptionUnit || selectedItem.unitId);

      setBom(prev => [...prev, {
        rmId: selectedItem.id,
        itemType: 'RAW_MATERIAL',
        name: selectedItem.name,
        code: selectedItem.code,
        categoryName: selectedItem.categoryName || 'General',
        unitPrice: Number(selectedItem.ratePerUnit || 0),
        consumption: 1,
        totalCost: Number(selectedItem.ratePerUnit || 0),
      ...
      // Non-Inventory item
      if (nonInventoryCosts.some(item => item.itemId === selectedItem.id)) {
        Swal.fire({
          title: `<span class="font-extrabold text-sm text-slate-800 dark:text-slate-100">Already Added</span>`,
          text: `Non-inventory item "${selectedItem.name}" is already in overhead costs.`,
          icon: 'warning',
          toast: true,
          position: 'top-end',
          showConfirmButton: false,
      ...
      setNonInventoryCosts(prev => [...prev, {
        itemId: selectedItem.id,
        name: selectedItem.name,
        code: selectedItem.code || '',
        categoryName: selectedItem.categoryName || 'Non-Inventory',
        displayUom: selectedItem.displayUom || 'pcs',
        cost: Number(selectedItem.ratePerUnit || selectedItem.cost || 0)
      }]);
      setSelectedItem(null);
    }
  };
  ...
  // Smart Rollover & Normalization: (>= 60 min -> +1 hr, >= 24 hr -> +1 day, >= 30 days -> +1 mo)
  const handleStageTimeBlur = (index) => {
    const updated = [...stages];
    const s = updated[index];
    if (!s) return;

    let m = Number(s.months || 0);
    let d = Number(s.days || 0);
    let h = Number(s.hours || 0);
    let min = Number(s.minutes || 0);

    // Roll over minutes (>= 60 min -> hours)
    if (min >= 60) {
      h += Math.floor(min / 60);
      min = min % 60;
    }

    // Roll over hours (>= 24 hr -> days)
    if (h >= 24) {
      d += Math.floor(h / 24);
      h = h % 24;
    }

    // Roll over days (>= 30 days -> months)
    if (d >= 30) {
      m += Math.floor(d / 30);
      d = d % 30;
    }

    updated[index].months = m;
    updated[index].days = d;
    updated[index].hours = h;
    updated[index].minutes = min;
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      stages: stages
        ...
        .map((s, idx) => {
          let m = Math.max(0, Math.round(Number(s.months || 0)));
          let d = Math.max(0, Math.round(Number(s.days || 0)));
          let h = Math.max(0, Math.round(Number(s.hours || 0)));
          let min = Math.max(0, Math.round(Number(s.minutes || 0)));
  ...
  return (
    <div className="w-full max-w-full px-4 sm:px-6 lg:px-8 py-5 space-y-4 mx-auto transition-all duration-300 animate__animated animate__fadeIn">
      ...
      <div className="hidden sm:flex justify-between items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 py-2.5 px-4 rounded-xl shadow-xs">
          ...
          return (
            <React.Fragment key={node.id}>
              <button
                ...
                className={`flex items-center gap-2.5 text-left focus:outline-none group px-2.5 py-1.5 rounded-xl transition-all ${
                  isCurrent 
              ...
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center font-extrabold text-xs transition-all ${
                  isCurrent 
                    ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-sm ring-2 ring-indigo-500/20' 
                    : isCompleted 
                    ? 'bg-emerald-500 text-white shadow-xs' 
                    : 'bg-slate-100 dark:bg-slate-800 border dark:border-slate-700 text-slate-500 group-hover:text-indigo-600'
                }`}>
                  {isCompleted ? <Check className="w-3.5 h-3.5 stroke-[3]" /> : node.num}
                </div>
                <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-start">
          ...
          {activeTab === 'operations' && (
            <div className="space-y-3.5 animate__animated animate__fadeIn">
              ...
              <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-xl overflow-hidden">
                <CardHeader className="py-2.5 px-4 border-b border-slate-100 dark:border-slate-800/80 flex flex-row items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 border border-indigo-200/60 dark:border-indigo-800/60">
                      {stages.length} / 14 Selected
                    </span>
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800/60">
                    ...
                    {STANDARD_FACTORY_STAGES.map((std, sidx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-2 rounded-xl text-left border transition-all relative flex flex-col justify-between h-[74px] group cursor-pointer ${
                            isAdded
              ...
              <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-xl overflow-hidden">
                ...
                <CardContent className="p-3">
                  {stages.length === 0 ? (
                    ...
                    <div className="space-y-2 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-[2px] before:bg-indigo-100 dark:before:bg-indigo-950/60">
                      {stages.map((st, idx) => {
                        ...
                        return (
                          <div 
                            ...
                            className={`flex gap-2 relative items-center group transition-all duration-150 ${
                          ...
                          >
                            ...
                            <div 
                              className="z-10 flex items-center justify-center w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 font-mono font-extrabold text-xs shrink-0 shadow-2xs cursor-grab active:cursor-grabbing hover:bg-indigo-50/50 transition-all"
                              title="Drag to reorder sequence"
                            >
                              #{idx + 1}
                            </div>
                            
                            <div className="flex-1 p-2.5 bg-slate-50/70 dark:bg-slate-950/40 hover:bg-slate-50 dark:hover:bg-slate-950/70 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 transition-all">
                              <div className="flex items-center gap-2">
                                <div 
                                  className="p-1 rounded text-slate-400 hover:text-indigo-600 cursor-grab active:cursor-grabbing transition-all"
                                  title="Drag to reorder sequence"
                                >
                              ...
                              <div className="flex items-center gap-2 justify-end">
                                <div className="flex gap-1 items-center text-2xs text-slate-500 font-bold font-mono">
                                  <div className="flex items-center gap-1 bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded-lg border border-slate-200 dark:border-slate-800" title="Months (0-36)">
                                    <Input 
                                      type="number" 
                                      className="w-9 h-6 p-0 text-center bg-transparent border-0 font-bold text-xs focus:ring-0 text-slate-900 dark:text-white" 
                                      min="0" 
                                      max="36"
                                      value={st.months} 
                                      onChange={(e) => handleStageTimeChange(idx, 'months', e.target.value)} 
                                      onBlur={() => handleStageTimeBlur(idx)}
                                    />
                                    <span className="text-slate-400 text-[10px]">Mo</span>
                                  </div>
                                  <div className="flex items-center gap-1 bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded-lg border border-slate-200 dark:border-slate-800" title="Days (0-29 days; 30+ rolls over to 1 Month)">
                                    <Input 
                                      type="number" 
                                      className="w-9 h-6 p-0 text-center bg-transparent border-0 font-bold text-xs focus:ring-0 text-slate-900 dark:text-white" 
                                      min="0" 
                                  ...
                                  <div className="flex items-center gap-1 bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded-lg border border-slate-200 dark:border-slate-800" title="Hours (0-23 hours; 24+ rolls over to 1 Day)">
                                    <Input 
                                      type="number" 
                                      className="w-9 h-6 p-0 text-center bg-transparent border-0 font-bold text-xs focus:ring-0 text-slate-900 dark:text-white" 
                                      min="0" 
                                  ...
                                  <div className="flex items-center gap-1 bg-white dark:bg-slate-900 px-1.5 py-0.5 rounded-lg border border-slate-200 dark:border-slate-800" title="Minutes (0-59 minutes; 60+ rolls over to 1 Hour)">
                                    <Input 
                                      type="number" 
                                      className="w-9 h-6 p-0 text-center bg-transparent border-0 font-bold text-xs focus:ring-0 text-slate-900 dark:text-white" 
                                      min="0" 
                                ...
                                <button 
                                  ...
                                  className="text-rose-500 hover:text-rose-600 p-1 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all"
                                  title="Remove stage"
                                >
                ...
                <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col sm:flex-row justify-between items-center gap-2.5">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={handlePrev}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      onClick={handleSubmit}
                      disabled={saving}
                      className="bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs py-2 px-6 shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer"
                    >
                      {saving ? (
                        <>
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
        ...
        <div className="space-y-4">
          <div className="bg-indigo-50/20 dark:bg-indigo-950/10 border border-indigo-100 dark:border-indigo-900/40 p-4 rounded-2xl space-y-3 sticky top-6 shadow-md transition-all">
            <div>
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div className="flex justify-between pt-0.5 font-extrabold text-indigo-600 dark:text-indigo-400 text-sm">
            ...
            {activeTab === 'operations' ? (
              <Button
                ...
                onClick={handleSubmit}
                disabled={saving}
                className="w-full bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white font-extrabold py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                {saving ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
