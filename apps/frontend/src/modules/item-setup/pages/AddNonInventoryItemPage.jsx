...
export default function AddNonInventoryItemPage() {
  const { id } = useParams();
  const isEditMode = !!id;
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    defaultValues: { unitId: 'pcs', ratePerUnit: 0, openingStock: 0, alertLevel: 0, code: '' }
  });

  const { data: existingData, isLoading: isFetching } = useQuery({
    queryKey: ['non-inventory-item', id],
    queryFn: async () => (await api.get(`/item-setup/non-inventory-item/${id}`)).data,
    enabled: isEditMode
  });
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(data => mutation.mutate(data))}>
        <Card className="dark:bg-[#111827] dark:border-slate-800">
          <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <input {...register('code', { required: true })} placeholder="e.g. NI-001" className="w-full px-3 py-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 dark:text-white" />
            ...
            <div className="space-y-2 md:col-span-2 lg:col-span-1">
              ...
              <textarea {...register('description')} rows="1" className="w-full px-3 py-2 border rounded-md dark:bg-slate-900 dark:border-slate-700 dark:text-white" />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="mt-6 flex space-x-4">
          ...
          <button type="button" onClick={() => navigate('/setup/non-inventory')} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-md font-medium">
            Back
          </button>
        </div>
      </form>
