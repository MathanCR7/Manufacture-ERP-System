...
export default function RMCategoryListPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => await api.delete(`/item-setup/rm-category/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['rm-categories'] })
  });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Raw Material Categories</h1>
      <Card className="dark:bg-[#111827] dark:border-slate-800">
        <CardContent className="p-0">
          <div className="p-4 flex justify-between items-center border-b dark:border-slate-700">
            <Link to="/setup/rm-category/add" className="flex items-center px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md text-sm font-medium">
                  ...
                  paginated.map((item, i) => (
                    <TableRow key={item.id} className="dark:border-slate-700">
                      ...
                      <TableCell className="text-right space-x-2">
                        <Link to={`/setup/rm-category/edit/${item.id}`} className="inline-block text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 p-1 rounded"><Edit className="w-4 h-4" /></Link>
