...
export default function NonInventoryItemListPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => await api.delete(`/item-setup/non-inventory-item/${id}`),
      ...
      <Card className="dark:bg-[#111827] dark:border-slate-800">
        <CardContent className="p-0">
          <div className="p-4 flex justify-between items-center border-b dark:border-slate-700">
            <Link to="/setup/non-inventory/add" className="flex items-center px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md text-sm font-medium">
                  ...
                  paginated.map((item, i) => (
                    <TableRow key={item.id} className="dark:border-slate-700">
                      ...
                      <TableCell className="text-right space-x-2">
                        <Link to={`/setup/non-inventory/edit/${item.id}`} className="inline-block text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 p-1 rounded"><Edit className="w-4 h-4" /></Link>
