...
export default function CustomerListPage() {
  ...
  const sortedAndFiltered = useMemo(() => {
    let result = (customers || []).filter(c => {
      ...
      const matchesBalanceType = balanceTypeFilter === 'ALL' || c.balanceType === balanceTypeFilter;
    ...
    return result;
  }, [customers, searchTerm, balanceTypeFilter, statusFilter, sortBy]);
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
              Manage client directory, credit limits, balances, and contact accounts.
            </p>
          </div>
        </div>
            ...
            <Table className="text-xs">
              <TableHeader className="bg-slate-50/80 dark:bg-slate-950/60 text-slate-600 dark:text-slate-400 font-bold border-b border-slate-200 dark:border-slate-800">
                <TableRow className="dark:border-slate-800">
                  ...
                  <TableHead className="py-2 px-3 text-[11px] uppercase tracking-wider font-bold">Email</TableHead>
                  ...
                  <TableHead className="py-2 px-3 text-[11px] uppercase tracking-wider font-bold">Note</TableHead>
                  <TableHead className="py-2 px-3 text-[11px] uppercase tracking-wider font-bold">Added By</TableHead>
              ...
              <TableBody>
                {paginatedCustomers.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center py-10 text-slate-400 dark:text-slate-500 font-medium">
                  ...
                  paginatedCustomers.map((customer, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="py-2 px-3 font-bold text-slate-900 dark:text-slate-100 text-xs">{customer.name}</TableCell>
                        <TableCell className="py-2 px-3 font-semibold text-slate-700 dark:text-slate-300 font-mono text-xs">{customer.phone}</TableCell>
                        <TableCell className="py-2 px-3 text-slate-500 dark:text-slate-400 text-xs">{customer.email || '—'}</TableCell>
                        <TableCell className="py-2 px-3 text-right font-bold text-slate-800 dark:text-slate-200 font-mono text-xs">
                          ₹{parseFloat(customer.creditLimit || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </TableCell>
                        <TableCell className="py-2 px-3 text-right font-bold text-slate-800 dark:text-slate-200 font-mono text-xs">
                          ₹{parseFloat(customer.openingBalance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </TableCell>
                        ...
                        <TableCell className="py-2 px-3 text-slate-500 dark:text-slate-400 truncate max-w-[200px] text-xs" title={customer.address}>
                          {customer.address || '—'}
                        </TableCell>
                        <TableCell className="py-2 px-3 text-slate-500 dark:text-slate-400 truncate max-w-[120px] text-xs" title={customer.note}>
                          {customer.note || '—'}
                        </TableCell>
                        <TableCell className="py-2 px-3 text-slate-600 dark:text-slate-400 font-medium text-[11px]">
                          {customer.user?.name || 'Admin'}
                        </TableCell>
                        <TableCell className="py-2 px-3 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end space-x-1">
                            <Link 
                              to={`/parties/customers/edit/${customer.id}`} 
                              title="Edit customer"
          ...
          <div className="px-3 py-1.5 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col sm:flex-row justify-between items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            <div>
              {sortedAndFiltered.length > 0 ? (
                <span>
                  Showing <strong className="text-slate-700 dark:text-slate-200">{(currentPage - 1) * itemsPerPage + 1}</strong> to <strong className="text-slate-700 dark:text-slate-200">{Math.min(currentPage * itemsPerPage, sortedAndFiltered.length)}</strong> of <strong className="text-slate-700 dark:text-slate-200">{sortedAndFiltered.length}</strong> customers
                </span>
              ) : (
                <span>0 customers found</span>
              )}
            </div>
