    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">
          {isAdmin ? 'Attendance Timeline' : 'My Check In / Out'}
        </h1>
        {isAdmin && (
          <div className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-3 py-1.5 rounded-lg">
            <Users className="w-4 h-4" /> Viewing all team attendance
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Records', value: logs.length, icon: Calendar, color: 'indigo' },
          { label: 'Total Hours', value: fmtDuration(totalHours), icon: Timer, color: 'emerald' },
          { label: 'Today Present', value: logs.filter(l => fmtDate(l.checkIn) === fmtDate(new Date())).length, icon: TrendingUp, color: 'purple' },
          { label: 'Status', value: status.isCheckedIn ? 'Checked In' : 'Checked Out', icon: Clock, color: status.isCheckedIn ? 'green' : 'slate' },
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{filtered.length} records</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {loading ? (
                ...
                <tr><td colSpan={isAdmin ? 7 : 6} className="text-center py-12 text-slate-400">No attendance records found.</td></tr>
              ) : filtered.map((log, i) => (
