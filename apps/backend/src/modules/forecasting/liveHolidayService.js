...
async function getLiveHolidays(year = 2026, forceRefresh = false) {
  ...
  // Sort and deduplicate by base name on same date (e.g. "Janmashtami" and "Janmashtami (Smarta)")
  liveData.sort((a, b) => a.date.localeCompare(b.date));
  const uniqueHolidays = [];
  const seenBaseKeys = new Map();

  for (const h of liveData) {
    // Strip parenthetical suffixes e.g. "Janmashtami (Smarta)" -> "janmashtami"
    const baseName = h.name
      .replace(/\s*\([^)]*\)/g, '')
      .replace(/[^a-zA-Z0-9]/g, '')
      .toLowerCase();
    const key = `${h.date}|${baseName}`;

    if (!seenBaseKeys.has(key)) {
      seenBaseKeys.set(key, h);
      uniqueHolidays.push(h);
    } else {
      // If the existing entry had parentheses and the new entry is cleaner without parentheses, pick the cleaner one
      const existing = seenBaseKeys.get(key);
      if (existing.name.includes('(') && !h.name.includes('(')) {
        const idx = uniqueHolidays.indexOf(existing);
        if (idx !== -1) {
          uniqueHolidays[idx] = h;
          seenBaseKeys.set(key, h);
        }
      }
    }
